...
while opcion != 5:
    ...
    if opcion == 1:
        ...
        username_var = input('Digite el nuevo nombre: ')
        password_var = input('Digite la nueva contraseña: ')
        usuario = Usuario(id_usuario=id_usuario_var, username= username_var, password= password_var)
        usuario_actualizdo = UsuarioDAO.actualizar(usuario)
        log.info(f'Usuario actualizado: {usuario_actualizdo}')
    elif opcion == 4:
